...
* Use the *cut* command to select out most interesting portions of each line by specifying "-d" (delimiter) and "-f" (field) - like: *grep "Authentication failure" /var/log/auth.log | grep “root" |  cut -f 4- -d ":"*   (field 4 onwards, where the delimiter between field is the ":" character). This can be very useful in extracting useful information from log data.
