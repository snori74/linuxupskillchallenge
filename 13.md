...
Because this user is not the first user created, they don't have the power to run `sudo` - which your user has by being a meber of the group `sudo`. 

So, to check which groups `fred` is a member of:

`sudo su fred`	- to "become fred", then:

`groups`

You can add `fred` to theis group with:
`sudo su fred`	- to "become fred", then:

And of course re-run `usermod -a -G sudo fred` and `groups` (as "fred") to check.
