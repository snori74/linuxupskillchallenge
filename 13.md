...
so your `ubuntu` user can do the same simply by prefixing the command with `sudo` - use this to add your support user to the group _root_, and then use the `group` command to check that it has worked.
