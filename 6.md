...
## Security
* As the sysadmin of this server, responsible for its security, you need to be aware that you've now increased the "attack surface" of your server. In addition to *ssh* on port 22, you are now exposig the *apache2* code to attack on port 80. The logs may reveal access from a wide range of visiting search engines - and attackers - and that’s perfectly normal.
